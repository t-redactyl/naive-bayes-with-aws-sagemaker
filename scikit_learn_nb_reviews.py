...
if __name__ == "__main__":
    ...
    train_data = os.path.join(train_data_dir, 'train_reviews.csv')
    full_data = os.path.join(train_data_dir, 'full_reviews.csv')
    ...
    train_x = train["review_body"].apply(lambda x: np.str_(x))
    
    tfidf_vect = TfidfVectorizer(max_features=5000)
    tfidf_vect.fit(full["review_body"].apply(lambda x: np.str_(x)))
